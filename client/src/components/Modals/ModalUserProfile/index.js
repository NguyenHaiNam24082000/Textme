    ...
    >
      ...
      <div className="relative">
        <div className="flex w-full gap-3 p-5 justify-between items-center absolute -top-[84px]">
          ...
          >
            <div className="flex items-end w-full mt-14">
        ...
        >
          ...
          <Tabs.Tab label="Mutual Groups">
            {profile && profile.channel.length > 0 ? (
              profile.channel.map((channel) => (
                ...
                >
                  <Group spacing="sm">
                    <div className="h-[58px] w-14 relative">
                      {channel.type === "GROUP" ? (
                        <Indicator
                          sx={{
                            indicator: {
          ...
          <Tabs.Tab label="Mutual Servers">
            {profile && profile.server.length > 0 ? (
              profile.server.map((server) => (
                <Group
                  key={server.id}
                  // position="apart"
                  className="w-full flex p-2 rounded-md cursor-pointer hover:bg-slate-300 group"
                  // onClick={() => setOpenedModalUserProfile(true)}
                >
                  <div className="h-[58px] w-14 relative">
                    <Avatar
